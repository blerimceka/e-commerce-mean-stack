...
router.post('/', async (req, res) => {
    ...
    if(!order) return res.send(404).send('The order cannot be created!')
...
router.put('/:id', async (req, res) => {
    ...
    if(!order) res.status(404).send('the order cannot be updated');
...
router.get('/get/totalsales', async (req, res) => {
    ...
    if(!totalSales) return res.status(400).send("The order cannot be generated")
